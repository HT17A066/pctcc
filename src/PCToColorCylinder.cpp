...
Filter<PointType>::getClassName;

typedef typename Filter<PointType>::PointCloud PointCloud;
typedef typename PointCloud::Ptr PointCloudPtr;
typedef typename PointCloud::ConstPtr PointCloudConstPtr;

typedef pcl::shared_ptr< CropPlane<PointType> > Ptr;
typedef pcl::shared_ptr< const CropPlane<PointType> > ConstPtr;
----------------------------------------
*/
...
RTC::ReturnCode_t PCToColorCylinder::onExecute(RTC::UniqueId ec_id)
{
  ...
  //フィルタの出力用の点群
    ...
    //フィルタ処理
    pcl::CropPlane<PointType> filter;
    filter.setInputCloud(cloud_filtered);
    filter.setPlane(params);
    //cout << "filter.getPlane(): " << endl << filter.getPlane() << endl;
    filter.setOffset(offset);
   // cout << "filter.getOffset(): " << endl << filter.getOffset() << endl;
    if (coefficients_plane->values[2] < 0) 
    {
        filter.setNegative(true); //これを有効にすると結果が反転
    }
    filter.filter(*cloud_filtered2);

    pcl::PointIndices::Ptr adove_plane(new pcl::PointIndices);
    filter.filter(adove_plane->indices);

    //平面以外の法線ベクトル群の抽出
    pcl::ExtractIndices<pcl::Normal> extract_normals;
    extract_normals.setInputCloud(cloud_normals);
    extract_normals.setNegative(false);
    extract_normals.setIndices(adove_plane);
